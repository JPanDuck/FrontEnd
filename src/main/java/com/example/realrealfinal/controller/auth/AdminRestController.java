...
import com.example.realrealfinal.domain.log.LogHistory;
...
import com.example.realrealfinal.util.LogUtils;
...
public class AdminRestController {
    ...
    public ResponseEntity<?> getLogs(@RequestParam(defaultValue = "50") int lines){
        try{
            List<String> logs = LogUtils.tail(lines);
            if(logs.isEmpty()){
                return ResponseEntity.ok("최근 " + lines + "줄 안에 WARN/ERROR 로그가 없습니다.");
            }
            return ResponseEntity.ok(logs);
        } catch (Exception e){
